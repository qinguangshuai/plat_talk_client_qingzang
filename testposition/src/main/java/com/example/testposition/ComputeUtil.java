    ...
    public static double gudaojuli(Location location, ArrayList<MyLocation> locs)
    {
        ...
        for (i = 0; i < locs.size() - 1; i++)
        {
            dd = GpsToDistance(location.getLongitude(), location.getLatitude(),
                    Double.valueOf(locs.get(i).getLongitude()), Double.valueOf(locs.get(i).getLongitude()),
                    Double.valueOf(locs.get(i+1).getLatitude()),Double.valueOf( locs.get(i+1).getLatitude()));
            if (dd <= jl)
            {
                jl = dd;
            }
