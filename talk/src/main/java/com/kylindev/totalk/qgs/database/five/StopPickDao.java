 ...
 * 停留车摘钩数据库
 */
public class StopPickDao {

    private StopPickOpenHelper helper;

    public StopPickDao(Context context) {
        helper = new StopPickOpenHelper(context);
    }

    /**
     * 数据库的增加方法
     */
    public boolean add(String gd,String lat, String lon,String time) {
...
//		db.execSQL("insert into info(name,phone) values(?,?)", new Object[]{NULL,phone});
        ...
        //实际底层原理 就是在组拼sql语句
        long result = db.insert("stoppick", null, values);
        db.close();
        if (result == -1) {
            //说明插入失败
            return false;
...
//		db.execSQL("delete from info where name=?", new Object[]{name});
     ...
     * 数据库的查询方法
     */
    public List<StopDataUser> find() {
        List<StopDataUser> personLists = new ArrayList<StopDataUser>();
        SQLiteDatabase db = helper.getWritableDatabase();
...
//		Cursor cursor = db.query("info", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
        ...
        Cursor cursor = db.query("stoppick", null, null, null, null, null, null);
        //对cursor 判断一下cursor
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                ...
                String time = cursor.getString(4);    //获取我们的name值
                StopDataUser dataUser = new StopDataUser();
