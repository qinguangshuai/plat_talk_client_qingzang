package com.kylindev.totalk.qgs.database.seven;
...
import com.kylindev.totalk.qgs.database.six.SixDataOpenHelper;
 ...
 * 人员号：07号
 */
public class SevenDataDao {

    private SevenDataOpenHelper helper;

    public SevenDataDao(Context context) {
        helper = new SevenDataOpenHelper(context);
    }
...
//		db.execSQL("insert into info(name,phone) values(?,?)", new Object[]{NULL,phone});
        ...
        values.put("lon", lon);
        //实际底层原理 就是在组拼sql语句
        long result = db.insert("sevenperson", null, values);
        db.close();
        if (result == -1) {
            //说明插入失败
            return false;
...
//		Cursor cursor = db.query("info", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
        ...
        Cursor cursor = db.query("sevenperson", null, null, null, null, null, null);
        //对cursor 判断一下cursor
