package com.kylindev.totalk.qgs.data232;
 ...
 * dmr  232数据保存
 */
public class DataDao232 {

    private DataOpenHelper232 helper;

    public DataDao232(Context context) {
        helper = new DataOpenHelper232(context);
    }
...
//		db.execSQL("insert into info(name,phone) values(?,?)", new Object[]{NULL,phone});
        ...
        values.put("lon", lon);
        //实际底层原理 就是在组拼sql语句
        long result = db.insert("DataOpenHelper232", null, values);
        db.close();
        if (result == -1) {
            //说明插入失败
            return false;
...
//		Cursor cursor = db.query("info", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
        ...
        Cursor cursor = db.query("DataOpenHelper232", null, null, null, null, null, null);
        //对cursor 判断一下cursor
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                ...
                String signalling = cursor.getString(2);    //获取我们的name值
                DataUser dataUser = new DataUser();

                System.out.println("time--" + time + "signalling--" + signalling);
                dataUser.setId(anInt);
                dataUser.setTime(time);
                dataUser.setSignalling(signalling);
