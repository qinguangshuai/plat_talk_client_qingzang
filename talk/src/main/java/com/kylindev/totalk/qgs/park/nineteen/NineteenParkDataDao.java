...
import com.kylindev.totalk.qgs.park.eighteen.EighteenParkOpenHelper;
 ...
 * 17道无数据时摘挂钩保存
 */
public class NineteenParkDataDao {

    private NineteenParkOpenHelper helper;

    public NineteenParkDataDao(Context context) {
        helper = new NineteenParkOpenHelper(context);
    }
...
//		db.execSQL("insert into info(name,phone) values(?,?)", new Object[]{NULL,phone});
        ...
        values.put("num", num);
        //实际底层原理 就是在组拼sql语句
        long result = db.insert("nineteenparkcar", null, values);
        db.close();
        if (result == -1) {
            //说明插入失败
            return false;
...
//		Cursor cursor = db.query("info", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
        ...
        Cursor cursor = db.query("nineteenparkcar", null, null, null, null, null, null);
        //对cursor 判断一下cursor
