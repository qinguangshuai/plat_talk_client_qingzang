package com.kylindev.totalk.qgs.database.ten;
...
import com.kylindev.totalk.qgs.database.nine.NineDataOpenHelper;
 ...
 * 人员号：10号
 */
public class TenDataDao {

    private TenDataOpenHelper helper;

    public TenDataDao(Context context) {
        helper = new TenDataOpenHelper(context);
    }
...
//		db.execSQL("insert into info(name,phone) values(?,?)", new Object[]{NULL,phone});
        ...
        values.put("lon", lon);
        //实际底层原理 就是在组拼sql语句
        long result = db.insert("tenperson", null, values);
        db.close();
        if (result == -1) {
            //说明插入失败
            return false;
...
//		Cursor cursor = db.query("info", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
        ...
        Cursor cursor = db.query("tenperson", null, null, null, null, null, null);
        //对cursor 判断一下cursor
