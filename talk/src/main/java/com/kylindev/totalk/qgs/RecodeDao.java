...
public class RecodeDao {
    ...
    private RecodeOpenHelper helper;

    public RecodeDao(Context context) {
        helper = new RecodeOpenHelper(context);
    }
...
//		db.execSQL("insert into info(name,phone) values(?,?)", new Object[]{NULL,phone});
        ...
        //实际底层原理 就是在组拼sql语句
        long result = db.insert("recode", null, values);
        db.close();
        if (result == -1) {
            //说明插入失败
            return false;
     ...
     * 数据库的查询方法
     */
    public List<SiveData> find() {
        List<SiveData> personLists = new ArrayList<SiveData>();
        SQLiteDatabase db = helper.getWritableDatabase();
...
//		Cursor cursor = db.query("info", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
        ...
        Cursor cursor = db.query("recode", null, null, null, null, null, null);
        //对cursor 判断一下cursor
