...
public class ReceiveDao {
    ...
    private ReceiveOpenHelper helper;

    public ReceiveDao(Context context) {
        helper = new ReceiveOpenHelper(context);
    }
...
//		db.execSQL("insert into info(name,phone) values(?,?)", new Object[]{NULL,phone});
        ...
        //实际底层原理 就是在组拼sql语句
        long result = db.insert("receive", null, values);
        db.close();
        if (result == -1) {
            //说明插入失败
            return false;
...
//		db.execSQL("delete from info where name=?", new Object[]{name});
         ...
         */
        //代表 影响了多少行
        int delete = db.delete("receive", "name=?", new String[]{name});
        db.close();
        return delete;
    }
     ...
     * 数据库的删除方法
     */
    public int update(String time, String name) {
        SQLiteDatabase db = helper.getWritableDatabase();
//		db.execSQL("update info set phone= ? where name= ?", new Object[]{newphone,name});
        ...
        values.put("time", time);
        int update = db.update("receive", values, "name=?", new String[]{name});
        db.close();
        return update;
    }
     ...
     * 数据库的删除方法
     */
    public List<ReceiveData> find() {
        List<ReceiveData> personLists = new ArrayList<ReceiveData>();
        SQLiteDatabase db = helper.getWritableDatabase();
...
//		Cursor cursor = db.query("info", new String[]{"phone"}, "name=?", new String[]{name}, null, null, null);
        ...
        Cursor cursor = db.query("receive", null, null, null, null, null, null);
        //对cursor 判断一下cursor
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                ...
                ReceiveData person = new ReceiveData();
